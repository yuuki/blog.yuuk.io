...
時系列データベースには、年単位でのメトリックのデータ保持と、1分以下の解像度でのメトリック書き込み性能が求められる。

時系列データベースには、大きく分類すると内部DBMS方式と外部DBMS方式、RDBMS拡張の3種類がある([^4],[^5])。RDBMS拡張については、ここでは外部DBMS方式に含むこととする。
外部DBMS方式は、時系列データベースに最適化されたデータベースエンジンを実装しており、後者は汎用DBMS上に時系列データベースとして動作させるためのアプリケーションを実装している。
内部DBMS方式は、性能とデータ保存効率がよいが、分散システムとしての信頼性の観点で、実験的な実装が多い。
...
Diamondは、Graphiteプロジェクト[^13]の時系列データベースであるWhisper[^14]データ構造に近いモデルを採用している。
Whisperデータ構造は、以下の2点に着目している。
...
(2) については、KVSで表現できる時系列データ構造やインデックス構造の限界がどこにあるかを調査する必要がある。
...
時系列データベースの機能として、現在のWhisperモデルに加えて、Prometheus[^7]が提供するようなラベルによる多次元データモデルがある。
