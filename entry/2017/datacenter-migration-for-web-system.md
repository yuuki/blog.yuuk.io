...
<!-- ## AWSへの移行 -->
<!--  -->
<!-- AWSへデータベースを移行する場合、Amazon DMSが使えるかもしれません。[https://aws.amazon.com/jp/dms/:title] -->
<!-- サポートされていれば、異種データベースの移行やバージョンの異なるデータベースへの移行が可能なようです。継続的なレプリケーションにも利用可能ということで、既存データの移行と新規データの移行、両方に利用できる可能性があります。 -->
