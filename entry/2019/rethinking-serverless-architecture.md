...
Category:
- Serverless
CustomPath: 2019/rethinking-serverless-architecture
Draft: true
---

2015年にAWS Lambdaが登場し，Lambdaのような関数を単位としてアプリケーションを実行する基盤をFunction as a Service(以下，FaaS)と呼ぶようになった．
...
[https://speakerdeck.com/naoya/serverless-architecture:title]
...
このように，サーバー上でソフトウェアを動作させることを意識して，プログラミングしたり，デプロイしたりすることを先程の"Cloud Programming Simplified"の論文ではサーバーフルコンピューティングと読んでいる．
...
アプリケーション実行単位がFunctionとなった結果，BaaS上で発生するイベントをFunctionに入力し，そのFunction内で別のBaaSのAPIを叩き，さらにそのイベントを契機に別のFunctionが起動するといったピタゴラスイッチのような構成がとられるようになった．
